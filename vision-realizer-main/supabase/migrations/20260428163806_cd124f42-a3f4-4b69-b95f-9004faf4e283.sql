-- Fix prior security warnings: set search_path on existing functions
...
-- Add FK from classes.homeroom_teacher_id to teachers
...
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    ...
    'transport_routes','events','announcements','health_records'
  ])
  LOOP
...
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    ...
    'fee_types','payments','staff','library_books','book_loans',
...
-- ============================================
-- RLS POLICIES — staff can view & manage most data; specific roles for sensitive tables
-- ============================================

-- Generic staff-read & admin/principal-write helper applied below per table

-- academic_years, grades, classes, subjects, fee_types, transport_routes, library_books, events, announcements:
-- staff can read; admin/principal can manage
DO $$
DECLARE t TEXT;
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    'academic_years','grades','classes','subjects','fee_types',
    'transport_routes','library_books','events','announcements'
  ])
  LOOP
...
-- teachers, staff: staff can view; admin/principal manage
...
-- students: staff read all; students/parents read own; admin/principal manage
...
-- student_parents, student_enrollments, teaching_assignments, schedules — staff read; admin manage
DO $$
DECLARE t TEXT;
BEGIN
  FOR t IN SELECT unnest(ARRAY[
...
-- attendance: staff read; teachers/admin/principal manage
...
-- exams, exam_results, report_cards
...
-- payments — accountant/admin manage; staff read; student/parent see own
...
-- book_loans: librarian/admin manage; staff read
...
-- discipline_records, health_records — staff read; admin/principal/teacher manage
DO $$
DECLARE t TEXT;
BEGIN
...
-- activity_logs: only admin can view; system writes via service role only
